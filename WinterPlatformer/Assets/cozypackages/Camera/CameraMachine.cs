...
using com.cozyhome.FiniteStateMachine;
using UnityEngine;
...
public class CameraMachine : MonoBehaviour
{
    ...
    public void ApplyOrbitPosition(float distrto) => ViewTransform.position = ComputeOrbitPosition(distrto);
    ...
    public Vector3 ComputeOrbitPosition(float disratio)
    {
        float Ratio = Vector3.Dot(ViewTransform.forward, Vector3.up);
        float Amount = DistanceCurve.Evaluate(Ratio) * DolleyDistance * disratio;
    ...
    public void ComputeEasingTime(float mouse, ref float ease, float max, float fdt)
    {
        if (mouse > 0F)
        {
            ease += fdt;
        ...
        {
            ease -= fdt;
            ease = Mathf.Max(ease, 0F);
        }
    }

    public float ComputeEasingAmount(float rto, float fdt) => EasingCurve.Evaluate(rto) * (MaxOrbitSpeed) * fdt;
